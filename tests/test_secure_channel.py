...
# Dummy AES and padding functions are assumed to be tested elsewhere.
# Here we focus on SecureSession logic.
...
def test_unwrap_response_missing_status_word(monkeypatch, session_params):
    session = SecureSession.open(**session_params)
    # Patch aes_cbc_decrypt to return too short plaintext
    def fake_decrypt(key, iv, data):
        return b"\x01"
    monkeypatch.setattr("keycard.secure_channel.aes_cbc_decrypt", fake_decrypt)
    monkeypatch.setattr("keycard.secure_channel.aes_cbc_encrypt", lambda *a, **k: b"Y" * 16)
